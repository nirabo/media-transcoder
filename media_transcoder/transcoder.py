...
SUPPORTED_EXTENSIONS = [".mp4", ".mkv", ".avi", ".mov", ".wmv", ".flv", ".webm", ".m4v", ".mpg", ".mpeg", ".ts", ".mts"]
...
class TranscoderDB:
    ...
    def record_job(self, input_file: Path, output_file: Path, codec: str, 
                  hardware_accel: bool, duration: float, status: str, 
    ...
    def update_media_library(self, file_path: Path, file_info: Dict[str, Any]) -> None:
        ...
        cursor.execute("SELECT file_path FROM media_library WHERE file_path = ?", (str(file_path),))
    ...
    def get_media_files(self, limit: int = 50, status: str = None) -> List[Dict[str, Any]]:
    ...
    def remove_media_file(self, file_path: Path) -> bool:
        ...
        cursor.execute("SELECT COUNT(*) FROM media_library WHERE file_path = ?", (str(file_path),))
        ...
        cursor.execute("DELETE FROM media_library WHERE file_path = ?", (str(file_path),))
        self.conn.commit()
        
        return True
        
    def get_media_info(self, file_path: Optional[Path] = None, limit: int = 50, status: str = None) -> List[Dict[str, Any]]:
        """Get media files from the library.
...
WEB_SAFE_CHARS = re.compile(r'[^a-zA-Z0-9.-]')  # Characters to replace for web-safe filenames
...
class TranscoderDB:
    ...
    def _init_schema(self) -> None:
        ...
        try:
            ...
            cursor.execute("ALTER TABLE media_library ADD COLUMN web_optimized INTEGER DEFAULT 0")
            self.conn.commit()
    
    def record_job(self, input_file: Path, output_file: Path, 
                  codec: str, hardware_accel: bool, 
        ...
        cursor.execute(
            """INSERT INTO transcoding_jobs 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (job_id, str(input_file), str(output_file), input_size, output_size,
             input_format, output_format, codec, int(hardware_accel),
             duration, status, error_message, datetime.datetime.now().isoformat(),
             params_json)
        )
        self.conn.commit()
        return job_id
    ...
    def update_media_library(self, file_path: Path, file_info: Dict[str, Any]) -> None:
        ...
        # Check if the file already exists in the database
        cursor.execute("SELECT 1 FROM media_library WHERE file_path = ?", (str(file_path),))
        exists = cursor.fetchone() is not None
...
def get_encoding_params(codec, use_hardware=True):
    ...
    if codec == NVIDIA_CODEC and use_hardware:
        ...
        return [
            ...
            # Audio parameters
            "-c:a", "aac",               # AAC audio codec for best compatibility
            "-b:a", "192k",             # Audio bitrate
            "-ac", "2",                 # 2 audio channels (stereo)
            "-ar", "48000",             # Audio sample rate
            
            # Subtitle handling
            "-c:s", "mov_text"           # Text-based subtitles compatible with MP4
        ]
    else:
        # CPU encoding parameters (fallback or default)
        return [
            # Video parameters for CPU encoding
            "-c:v", CPU_CODEC,           # Always use CPU codec for fallback
            "-preset", "medium",         # CPU preset (slower = better quality)
            "-crf", "22",                # Constant Rate Factor (balance of quality and size)
            "-profile:v", "main",        # Main profile for compatibility
            "-pix_fmt", "yuv420p",       # Pixel format for maximum compatibility
            "-movflags", "+faststart",   # Optimize for web streaming
...
class Transcoder:
    def __init__(self, args):
        ...
        self.skip_validation = args.skip_validation if hasattr(args, 'skip_validation') else False
        self.keep_originals = args.keep_originals if hasattr(args, 'keep_originals') else False
        self.web_safe_names = args.web_safe_names if hasattr(args, 'web_safe_names') else False
    ...
    def analyze_media_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if video_stream:
                ...
                is_web_optimized = self._is_web_optimized(probe_data, file_info['format'])
            ...
            console.print(f"[ERROR] Failed to analyze file: {file_path.name}", style="red")
            if e.stderr:
            ...
            if self.db:
                self.db.update_media_library(file_path, {
            ...
            console.print(f"[ERROR] {error_msg} analyzing {file_path.name}", style="red")
    ...
    def check_hardware_acceleration(self) -> None:
        ...
        software_only = hasattr(self, 'args') and hasattr(self.args, 'software_only') and self.args.software_only
        
        if software_only:
            self.codec = CPU_CODEC
            self.hwaccel_flags = []
            console.print("[INFO] Software-only mode enabled (hardware acceleration disabled)", style="yellow")
            return
        ...
        try:
            ...
            if has_nvidia_gpu:
                console.print(f"[DEBUG] NVIDIA GPU info:\n{nvidia_check.stdout.strip()}", style="dim")
            ...
            if has_cuda:
                console.print(f"[DEBUG] CUDA version:\n{cuda_check.stdout.strip()}", style="dim")
            ...
            nvenc_encoders = [line for line in encoders_output.split('\n') if 'nvenc' in line.lower()]
            console.print(f"[DEBUG] Available NVENC encoders:\n{chr(10).join(nvenc_encoders) if nvenc_encoders else 'None found'}", style="dim")
            ...
            console.print(f"[DEBUG] Available hwaccels:\n{hwaccels_check.stdout.strip()}", style="dim")
            ...
            if has_nvidia_gpu and has_cuda:
                ...
                if has_nvenc:
                    ...
                    console.print("[INFO] NVIDIA hardware acceleration enabled with h264_nvenc", style="green")
                    return
                else:
                    console.print("[WARNING] NVIDIA GPU detected but h264_nvenc encoder not available in ffmpeg", style="yellow")
            
            # Fall back to CPU encoding
            self.codec = CPU_CODEC
            self.hwaccel_flags = []
            console.print("[INFO] Using CPU encoding (no compatible hardware acceleration detected)", style="yellow")
            
        except Exception as e:
            console.print(f"[ERROR] Failed to check for hardware acceleration: {e}", style="red")
            self.codec = CPU_CODEC
            self.hwaccel_flags = []
    ...
    def find_video_files(self) -> List[Path]:
        ...
        if not target_path.exists():
            console.print(f"[ERROR] Target location does not exist: {self.target_location}", style="red")
            return []
        
        # Add debug output
        console.print(f"[DEBUG] Searching for video files in: {target_path}", style="dim")
        console.print(f"[DEBUG] Supported extensions: {SUPPORTED_EXTENSIONS}", style="dim")
            
        for ext in SUPPORTED_EXTENSIONS:
            pattern = f"**/*{ext}"
            console.print(f"[DEBUG] Looking for pattern: {pattern}", style="dim")
            files = list(target_path.glob(pattern))
            console.print(f"[DEBUG] Found {len(files)} files with extension {ext}", style="dim")
        ...
        if not video_files:
            ...
            console.print(f"[DEBUG] Found {len(video_files)} total video files", style="dim")
            for file in video_files[:5]:  # Show first 5 files
                console.print(f"[DEBUG] Found: {file}", style="dim")
            if len(video_files) > 5:
                console.print(f"[DEBUG] ... and {len(video_files) - 5} more files", style="dim")
    ...
    def transcode(self, video_file: Path) -> float:
        ...
        if not is_valid and not self.force:
            console.print(f"[ERROR] Skipping invalid file: {video_file.name}", style="red")
            console.print(f"[ERROR] Validation error: {error_message}", style="red")
            console.print(f"[TIP] Use --force to attempt transcoding anyway or --skip-validation to bypass checks", style="blue")
        ...
        if output_file.exists():
            console.print(f"[WARNING] Output file already exists: {output_file.name}", style="yellow")
            if not self.dry_run and not self.force:
                console.print(f"[INFO] Skipping: {video_file.name} (use --force to overwrite)", style="blue")
                ...
                console.print(f"[INFO] Force flag set - will overwrite: {output_file.name}", style="yellow")
        
        if self.dry_run:
            console.print(f"[DRY RUN] Would transcode: {video_file.name} to {output_file.name}", style="blue")
        ...
        if self.codec == NVIDIA_CODEC:
            try:
                elapsed_time = self._try_transcode(video_file, output_file, use_hardware=True)
                ...
                if "Function not implemented" in error_msg or "Impossible to convert" in error_msg:
                    console.print(f"[WARNING] Hardware acceleration failed, falling back to software encoding", style="yellow")
                    try:
                        elapsed_time = self._try_transcode(video_file, output_file, use_hardware=False)
                        ...
                        if self.db:
                            self.db.record_job(
                                ...
                                duration=0.0,
                                status="failed",
                                error_message=sw_e.stderr.decode() if sw_e.stderr else "Software encoding failed"
                            )
                        raise
                else:
                    # Record failure in database
            ...
            try:
                ...
                if self.db:
                    self.db.record_job(
                        ...
                        duration=0.0,
                        status="failed",
                        error_message=e.stderr.decode() if e.stderr else "Software encoding failed"
                    )
                raise
            
    def _try_transcode(self, video_file: Path, output_file: Path, use_hardware: bool = True) -> float:
        ...
        if hasattr(self, 'args') and hasattr(self.args, 'verbose') and self.args.verbose:
            cmd.append("-v")
            cmd.append("info")
        else:
            cmd.append("-v")
        ...
        if use_hardware and hasattr(self, 'hwaccel_flags') and self.hwaccel_flags:
            ...
            console.print(f"[DEBUG] Using hardware acceleration flags: {self.hwaccel_flags}", style="dim")
        ...
        if use_hardware and self.codec == NVIDIA_CODEC and hasattr(self, 'nvenc_params'):
            ...
            try:
                ...
                has_subtitles = any(stream.get('codec_type') == 'subtitle' for stream in probe_data.get('streams', []))
                if has_subtitles:
                    cmd.extend(["-c:s", "mov_text"])  # Text-based subtitles compatible with MP4
                else:
            ...
            console.print(f"[DEBUG] Using NVENC parameters: {self.nvenc_params}", style="dim")
        else:
            # Get standard encoding parameters
            encoding_params = get_encoding_params(CPU_CODEC if not use_hardware else self.codec, use_hardware)
            cmd.extend(encoding_params)
            console.print(f"[DEBUG] Using standard encoding parameters: {encoding_params}", style="dim")
        ...
        console.print(f"[INFO] Transcoding: {video_file.name} ({'hardware' if use_hardware else 'software'})", style="blue")
        
        start_time = time.time()
        try:
            result = subprocess.run(cmd, check=True, capture_output=True)
            elapsed_time = time.time() - start_time
            console.print(f"[SUCCESS] Transcoded: {video_file.name} to {output_file.name}", style="green")
            return elapsed_time
        except subprocess.CalledProcessError as e:
            ...
            if e.stderr:
                ...
                console.print(f"[ERROR] FFmpeg error: {' '.join(error_lines)}", style="red")
                
                # Provide helpful suggestions based on error message
                if "moov atom not found" in error_msg:
                    console.print(f"[TIP] File appears to be corrupted. Try repairing it with: ffmpeg -i {video_file.name} -c copy repaired_{video_file.name}", style="blue")
                elif "Invalid data found when processing input" in error_msg:
                    console.print(f"[TIP] File format may be unsupported or corrupted", style="blue")
    ...
    def rename_file(self, video_file: Path) -> None:
        ...
        if self.dry_run:
            console.print(f"[DRY RUN] Would rename: {video_file.name} to {video_file.name}.backup", style="blue")
            return
            
        if self.keep_originals:
            console.print(f"[INFO] Keeping original file: {video_file.name}", style="blue")
            return
            
        try:
            backup_file = video_file.with_suffix(f"{video_file.suffix}.backup")
            video_file.rename(backup_file)
            console.print(f"[INFO] Renamed original file to: {backup_file.name}", style="blue")
        except Exception as e:
            console.print(f"[ERROR] Failed to rename original file: {str(e)}", style="red")
    ...
    def _is_web_optimized(self, probe_data: Dict[str, Any], format_name: str) -> bool:
        ...
        if format_name == "mp4":
            ...
            if format_tags.get("major_brand") == "isom" and "mov_text" in str(probe_data):
                return True
        ...
        if format_name == "matroska":
            return video_codec in web_friendly_codecs and audio_codec in web_friendly_audio
    ...
    def repair_file(self, video_file: Path) -> Optional[Path]:
        ...
        if not video_file.exists():
            console.print(f"[ERROR] File does not exist: {video_file.name}", style="red")
            return None
            
        console.print(f"[INFO] Attempting to repair file: {video_file.name}", style="blue")
            ...
            try:
                ...
                if result:
                    ...
                    if is_valid:
                        ...
                        final_file = video_file.with_name(f"{video_file.stem}_repaired{video_file.suffix}")
                        shutil.copy2(repaired_file, final_file)
                        console.print(f"[SUCCESS] File repaired successfully: {final_file.name}", style="green")
                        ...
                        console.print(f"[ERROR] Repair attempt succeeded but file is still invalid", style="red")
                        repaired_file.unlink(missing_ok=True)
            except Exception as e:
                console.print(f"[ERROR] Repair method failed: {str(e)}", style="red")
        
        console.print(f"[ERROR] All repair attempts failed for: {video_file.name}", style="red")
        return None
    ...
    def _repair_remux(self, video_file: Path, output_file: Path) -> bool:
        ...
        """
        console.print(f"[INFO] Trying repair by remuxing: {video_file.name}", style="blue")
    ...
    def _repair_rebuild_index(self, video_file: Path, output_file: Path) -> bool:
        ...
        Returns:
            bool: True if repair was successful, False otherwise
        """
        console.print(f"[INFO] Trying repair by rebuilding index: {video_file.name}", style="blue")
    ...
    def _repair_copy_streams(self, video_file: Path, output_file: Path) -> bool:
        ...
        Returns:
            bool: True if repair was successful, False otherwise
        """
        console.print(f"[INFO] Trying repair by copying streams: {video_file.name}", style="blue")
        ...
        if repaired_file.exists():
            console.print(f"[INFO] Repaired file already exists: {repaired_file.name}", style="blue")
        ...
        try:
            result = subprocess.run(cmd, check=True, capture_output=True)
            console.print(f"[SUCCESS] Repaired file: {video_file.name} -> {repaired_file.name}", style="green")
            return repaired_file
        except subprocess.CalledProcessError as e:
            console.print(f"[ERROR] Failed to repair file: {video_file.name}", style="red")
            if e.stderr:
                error_msg = e.stderr.decode()
                error_lines = error_msg.split('\n')[:2]
                console.print(f"[ERROR] FFmpeg repair error: {' '.join(error_lines)}", style="red")
            return None
    
    def _process_batch_files(self, batch: List[Path], progress) -> None:
        """Process a batch of video files with a shared progress bar."""
        console.print(f"[INFO] Starting batch processing of {len(batch)} files", style="blue")
        ...
        for video_file in batch:
            ...
            if "_jellyfin.mp4" in video_file.name:
                console.print(f"[INFO] Skipping already transcoded file: {video_file.name}", style="blue")
                skipped += 1
            ...
            try:
                ...
                console.print(f"[ERROR] Exception during transcoding: {str(e)}", style="red")
                
                # Try to repair the file if enabled
                if self.args.repair:
                    repaired_file = self.repair_file(video_file)
                    if repaired_file:
                        console.print(f"[INFO] Attempting to transcode repaired file", style="blue")
                        try:
                            ...
                            if elapsed_time > 0:
                                console.print(f"[SUCCESS] Transcoded repaired file successfully", style="green")
                                successful += 1
                            else:
                                failed += 1
                        except Exception as repair_e:
                            console.print(f"[ERROR] Failed to transcode repaired file: {str(repair_e)}", style="red")
                            failed += 1
                    else:
                        failed += 1
                else:
        ...
        console.print(f"[INFO] Batch results: {successful} successful, {failed} failed, {skipped} skipped", style="blue")
        console.print(f"[INFO] Completed batch processing of {len(batch)} files", style="green")
    
    def process_files_parallel(self, video_files: List[Path], max_workers: int = 2) -> None:
        ...
        with Progress(
            ...
            batch_task = progress.add_task("[cyan]Processing batches...", total=len(batches))
    ...
    def generate_report(self) -> None:
        ...
        console.print(f"Total processing time: {self.total_real_time:.3f} seconds", style="blue")
        
        if self.total_files > 0:
            avg_time = self.total_real_time / self.total_files
            console.print(f"Average time per file: {avg_time:.2f} seconds", style="blue")
        ...
        if self.stats["total_size_before"] > 0 and self.stats["total_size_after"] > 0:
            ...
            reduction = (1 - (size_after_mb / size_before_mb)) * 100 if size_before_mb > 0 else 0
            ...
            console.print(f"Size reduction: {reduction:.2f}%", style="green" if reduction > 0 else "red")
        
        # Display processing statistics
        console.print(f"Files processed: {self.stats['processed']}", style="blue")
        console.print(f"Successfully transcoded: {self.stats['successful']}", style="green")
    ...
    def clean_media_library(self, directory: Path) -> None:
        ...
        console.print(f"[INFO] Scanning directory for media files: {directory}", style="blue")
        ...
        console.print(f"[INFO] Found {len(media_files)} media files to analyze", style="blue")
        ...
        with Progress(
            ...
            for media_file in media_files:
                ...
                if is_valid:
                    ...
                    stats["invalid"] += 1
                    console.print(f"[WARNING] Invalid file: {media_file.name} - {error_message}", style="yellow")
                    
                    # Try to repair the file if repair flag is set
                    if self.args.repair:
                        console.print(f"[INFO] Attempting to repair: {media_file.name}", style="blue")
                        repaired_file = self.repair_file(media_file)
                        
                        if repaired_file:
                            stats["repaired"] += 1
                            console.print(f"[SUCCESS] Repaired file: {media_file.name}", style="green")
                            ...
                            if self.db:
                                ...
                                if file_info:
                                    ...
                                    file_info["notes"] = f"Repaired from {media_file.name}"
                                    self.db.update_media_library(repaired_file, file_info)
                        else:
                            console.print(f"[ERROR] Could not repair: {media_file.name}", style="red")
                            ...
                            if self.force:
                                try:
                                    media_file.unlink()
                                    stats["removed"] += 1
                                    console.print(f"[INFO] Removed corrupted file: {media_file.name}", style="blue")
                                    
                                    # Remove from database if available
                                    if self.db and hasattr(self.db, 'remove_media_file'):
                                        self.db.remove_media_file(media_file)
                                except Exception as e:
                                    console.print(f"[ERROR] Failed to remove file: {media_file.name} - {str(e)}", style="red")
                    else:
                        ...
                        if self.force:
                            try:
                                media_file.unlink()
                                stats["removed"] += 1
                                console.print(f"[INFO] Removed invalid file: {media_file.name}", style="blue")
                            ...
                            console.print(f"[TIP] Use --force to remove invalid files or --repair to attempt repair", style="blue")
            ...
            for media_file in media_files:
                ...
                if file_info:
                    ...
                    if self.args.verbose if hasattr(self.args, 'verbose') else False:
                        console.print(f"[INFO] Analyzed: {media_file.name}", style="blue")
                        for key, value in file_info.items():
                            console.print(f"  {key}: {value}")
                else:
                    stats["failed"] += 1
                    console.print(f"[ERROR] Failed to analyze: {media_file.name}", style="red")
    ...
    def run_tests(self) -> bool:
        ...
        if self.codec in [NVIDIA_CODEC, CPU_CODEC]:
            console.print("[TEST PASSED] Hardware acceleration check works", style="green")
        else:
            console.print("[TEST FAILED] Hardware acceleration check failed", style="red")
            tests_passed = False
        ...
        if self.codec == NVIDIA_CODEC:
            try:
                ...
                if "b:v" in result.stdout and "preset" in result.stdout:
                    console.print("[TEST PASSED] FFmpeg supports NVENC parameters", style="green")
                else:
                    console.print("[TEST WARNING] FFmpeg may not support all NVENC parameters", style="yellow")
            except Exception as e:
                console.print(f"[TEST WARNING] Could not verify NVENC parameter support: {e}", style="yellow")
        ...
        try:
            ...
            if "crf" in result.stdout and "preset" in result.stdout:
                console.print("[TEST PASSED] FFmpeg supports x264 parameters", style="green")
            else:
                console.print("[TEST WARNING] FFmpeg may not support all x264 parameters", style="yellow")
        except Exception as e:
            console.print(f"[TEST WARNING] Could not verify x264 parameter support: {e}", style="yellow")
    ...
    def run(self) -> int:
        ...
        # Handle clean mode
        if self.clean_mode:
            console.print(f"[INFO] Running in clean mode for {self.target_location}", style="blue")
            self.clean_media_library(Path(self.target_location))
            return 0
            
        # Handle analyze-only mode
        if self.analyze_only:
            console.print(f"[INFO] Running in analyze-only mode for {self.target_location}", style="blue")
        ...
        console.print(f"[INFO] Found {len(video_files)} video files to process", style="blue")
            
        if self.dry_run:
            console.print("[INFO] Running in dry-run mode, no files will be modified", style="blue")
...
def main():
    ...
    parser.add_argument(
        "-t", "--target",
        help="Target location to operate on",
        type=str
    )
    parser.add_argument(
        "-n", "--dry-run",
    ...
    parser.add_argument(
        "-d", "--destination",
        help="Specify a destination for output files",
        type=str
    )
    parser.add_argument(
        "-T", "--test",
    ...
    parser.add_argument(
        "--analyze",
        help="Analyze media files without transcoding",
        action="store_true"
    )
    parser.add_argument(
        "--clean",
    ...
    parser.add_argument(
        "--list-media",
        help="List media files in the database",
        action="store_true"
    )
    parser.add_argument(
        "--media-limit",
    ...
    if args.list_jobs or args.list_media or args.reset_db:
        db_path = args.db_path if hasattr(args, 'db_path') and args.db_path else DEFAULT_DB_PATH
        db = TranscoderDB(db_path)
        
        if args.reset_db:
            console.print(f"[INFO] Resetting database: {db_path}", style="yellow")
            db.reset_database()
            console.print("[INFO] Database has been reset. All records have been deleted.", style="green")
            db.close()
            return 0
        
        if args.list_jobs:
            console.print(f"[INFO] Listing recent transcoding jobs from database: {db_path}", style="blue")
            jobs = db.get_recent_jobs(limit=args.job_limit)
            
            if not jobs:
                console.print("[INFO] No transcoding jobs found in the database", style="yellow")
            else:
                table = Table(title=f"Recent Transcoding Jobs (showing {len(jobs)} of {args.job_limit} requested)")
        ...
        if args.list_media:
            console.print(f"[INFO] Listing media files from database: {db_path}", style="blue")
            media_files = db.get_media_files(limit=args.media_limit)
            
            if not media_files:
                console.print("[INFO] No media files found in the database", style="yellow")
            else:
                table = Table(title=f"Media Library (showing {len(media_files)} of {args.media_limit} requested)")
                ...
                for media in media_files:
                    ...
                    duration = f"{media.get('duration', 0):.2f}s" if media.get('duration') else "N/A"
